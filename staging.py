...
# Fonction pour déplacer un fichier vers le répertoire history
def move_file_to_history(file_path):
    history_path = "hdfs://localhost:9000/transactions/raw/history"
    # Créer le répertoire history s'il n'existe pas
    fs = spark._jvm.org.apache.hadoop.fs.FileSystem.get(spark._jsc.hadoopConfiguration())
    history_dir = spark._jvm.org.apache.hadoop.fs.Path(history_path)
    
    if not fs.exists(history_dir):
        fs.mkdirs(history_dir)
    
    # Définir les chemins source et destination
    source_path = spark._jvm.org.apache.hadoop.fs.Path(file_path)
    dest_path = spark._jvm.org.apache.hadoop.fs.Path(f"{history_path}/{os.path.basename(file_path)}")
    
    # Déplacer le fichier
    fs.rename(source_path, dest_path)
    print(f"File moved to history: {dest_path}")
...
def process_json(file_path):
    ...
    # Lire le fichier JSON en mode multiline (chaque objet JSON est sur une ligne séparée)
    df = spark.read.option("multiline", "true").json(file_path)
    
    # Afficher les colonnes pour vérifier le format
    print(f"Columns in {file_path}: {df.columns}")
    
    # Si _corrupt_record est la seule colonne, cela signifie que le fichier est malformé
    if "_corrupt_record" in df.columns:
        print("Warning: The file contains malformed JSON. Skipping this file.")
        return None

    # Si le fichier a des données valides, on continue avec le traitement
    df = df.select(
...
# Fonction de traitement des fichiers et enregistrement dans Parquet
def process_files(file_type):
    file_paths = []
    parquet_path = "/transactions/staging"  # Répertoire de staging Parquet dans HDFS
    ...
    if file_type == "json":
        ...
        parquet_table_path = f"{parquet_path}/result_json"
    elif file_type == "txt":
        ...
        parquet_table_path = f"{parquet_path}/result_txt"
    elif file_type == "csv":
        ...
        parquet_table_path = f"{parquet_path}/result_csv"

    # Traiter chaque fichier et l'enregistrer dans Parquet
    for file_path in file_paths:
        ...
        try:
            ...
            if df is not None:
                ...
                # Enregistrer les données dans Parquet au chemin spécifié
                df.write.format("parquet").mode("append").save(f"hdfs://localhost:9000{parquet_table_path}")
                print(f"Data saved in Parquet at {parquet_table_path}")

                # Déplacer le fichier traité dans le répertoire history
                move_file_to_history(file_path)
