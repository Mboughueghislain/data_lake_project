...
customers = [
    ...
    for i in range(1, 76)  # Fix range to generate 75 customers
]
...
products = [
    ...
    for i in range(1, 21)  # Fix range to generate 20 products
]
...
transaction_sql = "INSERT INTO transactions (id, customer_id, total_amount, currency, payment_method, transaction_status, shipping_address, promo_code, transaction_type, shipping_fee, discount) VALUES\n"
transaction_sql += ",\n".join([
    f"({t['id']}, {t['customer_id']}, {t['total_amount']}, '{t['currency']}', '{t['payment_method']}', '{t['transaction_status']}', '{t['shipping_address'].replace('\'', '\\\'')}', "
    f"{f'\'{t[\'promo_code\']}\' ' if t['promo_code'] else 'NULL'}, '{t['transaction_type']}', {t['shipping_fee']}, {t['discount']})" for t in transactions
]) + ";"

transaction_detail_sql = "INSERT INTO transaction_details (transaction_id, product_id, quantity, unit_price, total) VALUES\n"
