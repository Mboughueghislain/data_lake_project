...
customer_sql += ",\n".join([
    f"({c['id']}, '{c['first_name']}', '{c['last_name']}', '{c['email']}', '{c['phone']}', '{c['address']}')"
    for c in customers
]) + ";"

product_sql = "INSERT INTO products (id, product_name, description, price, available_stock, category) VALUES\n"
product_sql += ",\n".join([
    f"({p['id']}, '{p['product_name']}', '{p['description']}', {p['price']}, {p['available_stock']}, '{p['category']}')"
    for p in products
]) + ";"
...
transaction_sql += ",\n".join([
    (
        ...
        f"'{t['transaction_status']}', '{t['shipping_address']}', "
        f"{'NULL' if t['promo_code'] is None else {t["promo_code\"]}\"'}, "
