...
transaction_sql += ",\n".join([f"({t['id']}, {t['customer_id']}, {t['total_amount']}, '{t['currency']}', '{t['payment_method']}', '{t['transaction_status']}', '{t['shipping_address'].replace('\'', '\\'')}', "f"{f'\'{t[\'promo_code\']}\' ' if t['promo_code'] else 'NULL'}, '{t['transaction_type']}', {t['shipping_fee']}, {t['discount']})" for t in transactions]) + ";"
