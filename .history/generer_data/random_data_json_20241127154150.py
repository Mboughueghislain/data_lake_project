...
# Configuration HDFS
HDFS_URL = "hdfs://localhost:9092"
OUTPUT_DIR_TXT = "/transactions/imports/txt"
OUTPUT_DIR_JSON = "/transactions/imports/json"
...
# Sauvegarder les données dans un fichier texte au format CSV-like et l'envoyer à HDFS
txt_file_name = f"social_media_{now.strftime('%Y%m%d%H%M%S')}.txt"
txt_file_path = f"{OUTPUT_DIR_TXT}/{txt_file_name}"

with hdfs_client.write(txt_file_path, encoding='utf-8') as txt_file:
    # Écrire l'en-tête
...
print(f"Les données fictives ont été sauvegardées dans {txt_file_path} au format CSV-like.")

# Sauvegarder les données dans un fichier JSON et l'envoyer à HDFS
json_file_name = f"social_media_{now.strftime('%Y%m%d%H%M%S')}.json"
json_file_path = f"{OUTPUT_DIR_JSON}/{json_file_name}"

with hdfs_client.write(json_file_path, encoding='utf-8') as json_file:
    json.dump(social_media_data, json_file, indent=4)

print(f"Les données fictives ont été sauvegardées dans {json_file_path} au format JSON.")